...
class DeepVO(nn.Module):
    def __init__(self, im_h, im_w, batchNorm=True):
        super(DeepVO, self).__init__()
        # CNN
        self.clip = par.clip
        self.flow = FlowNet(batchNorm=batchNorm)
        # Comput the shape based on diff image size
        i = Variable(torch.zeros(1, 6, im_h, im_w))
        i = self.flow(i)

        # RNN
        self.rnn = nn.LSTM(input_size=int(np.prod(i.size())), hidden_size=1000, num_layers=2, batch_first=True)
        self.rnn_dropout = nn.Dropout(par.rnn_dropout)
    ...
    def forward(self, x):
        """
        x: (b_size, seq_len, 3, w, h)
        return: sequence of poses. (b_size, seq_len, 6)
        """
        x = torch.cat((x[:, :-1], x[:, 1:]), dim=2)  # (B, L, 6, w, h)
        batch_size = x.size(0)
        seq_len = x.size(1)
        # CNN
    ...
    def get_loss(self, x, y):
        ...
        angle_loss = criterion(predicted[:, :, :3], y[:, :, :3]) / b
        translation_loss = criterion(predicted[:, :, 3:], y[:, :, 3:]) / b
